...
const progressBarCurrent = document.querySelector('.progress-duration--curent')
...
const createPlayList = () => {
  ...
  playList.forEach(el => {
    ...
    item.classList.add('small-button')
    item.classList.add('play-button')
...
const addPlayListListener = () => {
  ...
  songs.forEach(song => {
    song.addEventListener('click', () => {
      songs.forEach(song => {
        song.classList.remove('pause-button')
      })
      song.classList.add('pause-button')
...
const changeActiveItem = (activeItem) => {
  ...
  items[activeItem].style.color = `#c5b358`
...
const getTime = (time) => {
  ...
  let min = (Math.floor(newTime / 60)).toString().padStart(2, 0)
  let hours = (Math.floor(min / 60)).toString().padStart(2, 0)
